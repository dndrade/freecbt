...
import type { SecureBackupRecoveryKey } from "@/src/platform/backup/recovery-key";
...
describe("secureBackup createBackup", () => {
  test("prunes old backups after a successful verified write", async () => {
    ...
    files.read.mockImplementation(
      async (_fileUri: string) => files.write.mock.calls.at(-1)?.[1] ?? "",
    );

    await backup.createBackup(fixtureArchive());

    expect(files.write).toHaveBeenCalledTimes(1);
    expect(files.listFiles).toHaveBeenCalledWith("file:///backups");

    const writeOrder = files.write.mock.invocationCallOrder[0];
    const listOrder = files.listFiles.mock.invocationCallOrder[0];
  ...
  test("does not prune old backups when the backup write fails", async () => {
    ...
    await expect(backup.createBackup(fixtureArchive())).rejects.toBe(
      writeError,
    );
...
describe("secureBackup restoreBackupFile", () => {
  test("reads and restores an Archive-v3 file", async () => {
    ...
    expect(files.read).toHaveBeenCalledWith("file:///backups/archive-v3");
    expect(keys.create).not.toHaveBeenCalled();
  });
  ...
  test("reads and restores a legacy Archive-v2 file without recovery-key access", async () => {
    ...
    expect(files.read).toHaveBeenCalledWith("file:///backups/archive-v2");
    expect(keys.read).not.toHaveBeenCalled();
    expect(keys.create).not.toHaveBeenCalled();
  });
  ...
  test("restores Archive-v3 file with a manually supplied recovery key", async () => {
    ...
    await expect(
      backup.restoreBackupFile("file:///backups/archive-v3", RECOVERY_KEY),
    ).resolves.toEqual(fixtureArchive());
