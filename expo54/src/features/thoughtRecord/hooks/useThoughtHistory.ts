...
export function useThoughtHistory() {
  const [thoughts, setThoughts] = useState<readonly Thought.Thought[]>([]),
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const thoughts = await thoughtsService(
        DistortionData,
        await ensureThoughtRecordReady(),
      ).readAll();
      if (currentRequest === request.current) setThoughts(thoughts);
    } catch (error) {
      if (currentRequest === request.current)
        setError(error instanceof Error ? error : new Error(String(error)));
    } finally {
      if (currentRequest === request.current) setIsLoading(false);
    }
  }, []);
