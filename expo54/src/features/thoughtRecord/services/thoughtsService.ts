...
export function thoughtsService(data: Distortion.Data, db: SQLiteDatabase) {
  ...
  const toThought = (row: Row): Thought.Thought =>
    Thought.Thought.parse({
      uuid: row.uuid,
      automaticThought: row.automatic_thought,
      cognitiveDistortions: distortions.fromSlugSet.decode(
        new Set(JSON.parse(row.cognitive_distortions)),
      ),
  ...
  async function read(uuid: Thought.Id) {
    const row = await db.getFirstAsync<Row>(
      "SELECT * FROM thoughts WHERE uuid = ?",
      [uuid],
    );
  ...
  async function write(thought: Thought.Thought) {
    await db.runAsync(
      `INSERT INTO thoughts (uuid, automatic_thought, cognitive_distortions, challenge,
                                                 alternative_thought, created_at, updated_at)
                           VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(uuid) DO
                UPDATE SET automatic_thought=excluded.automatic_thought, cognitive_distortions=excluded.cognitive_distortions, challenge=excluded.challenge, alternative_thought=excluded.alternative_thought, updated_at=excluded.updated_at`,
      [
        thought.uuid,
        thought.automaticThought,
        JSON.stringify([...thought.cognitiveDistortions].map((d) => d.slug)),
        thought.challenge,
  ...
  async function remove(uuid: Thought.Id) {
    await db.runAsync("DELETE FROM thoughts WHERE uuid = ?", [uuid]);
  }
  ...
  async function persistSubmittedThought(
    ...
    try {
      if (_.isEqual(await read(submissionId), thought)) return;
    } catch (error) {
      if (
    ...
    throw new Error(
      `persistSubmittedThought: conflicting record already exists at ${submissionId} for submission ${submissionId}`,
    );
  }
