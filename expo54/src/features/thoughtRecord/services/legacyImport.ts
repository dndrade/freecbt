...
const outboxStatuses: readonly ThoughtSaveOutboxStatus[] = [
  "insertion-pending",
  "pending",
  "uncertain",
  "active",
...
async function importHomeDraft(
  data: Distortion.Data,
  storage: AsyncStorageStatic,
  mmkv: Pick<MmkvLike, "getString" | "set">,
) {
  if (mmkv.getString(WIZARD_SESSION) !== undefined) return;
  let draft;
  try {
    draft = await homeThoughtDraft(data, storage).read();
  } catch (error) {
    ...
    mmkv.set(
      ...
      JSON.stringify({
        state: {
          ...
          selectedDistortionSlugs: Array.from(
            draft.spec.cognitiveDistortions,
...
export async function importLegacyAsyncStorageData(
  data: Distortion.Data,
  db: SQLiteDatabase,
  storage: AsyncStorageStatic,
  mmkv: Pick<MmkvLike, "getString" | "set">,
) {
  if (mmkv.getString(IMPORTED) === "true")
    return importHomeDraft(data, storage, mmkv);

  const parser = Thought.createParsers(data),
    thoughts = thoughtsService(data, db),
    outbox = thoughtSaveOutboxService(data, db);
  const sameOutboxRecord = (
    left: ThoughtSaveOutboxRecord,
    right: ThoughtSaveOutboxRecord,
  ) =>
    left.submissionId === right.submissionId &&
    JSON.stringify(parser.toJson.encode(left.thought)) ===
      JSON.stringify(parser.toJson.encode(right.thought)) &&
  ...
  for (const [, value] of await storage.multiGet(
    (await storage.getAllKeys()).filter(Thought.isKey),
  ))
    if (value !== null) {
      let thought: Thought.Thought;
      try {
        thought = parser.fromString.decode(value);
      } catch {
        continue;
      }
    ...
    for (const record of records) {
      ...
      try {
        ...
        if (
          ...
          !Number.isInteger(r.sourceDraftRevision) ||
          (r.sourceDraftRevision as number) < 0 ||
          ...
          typeof r.retryRequested !== "boolean" ||
          typeof r.thoughtPersisted !== "boolean" ||
          typeof r.updatedAt !== "string" ||
          !outboxStatuses.includes(r.status as ThoughtSaveOutboxStatus)
        )
          continue;
        const submissionId = Thought.Id.decode(r.submissionId),
          thought = parser.toJson.decode(r.thought as never),
          lastAttemptAt = new Date(r.lastAttemptAt),
          updatedAt = new Date(r.updatedAt);
        if (
          Number.isNaN(lastAttemptAt.getTime()) ||
          Number.isNaN(updatedAt.getTime()) ||
          submissionId !== thought.uuid ||
          (r.status === "cleanup-failed" && !r.thoughtPersisted)
        )
          continue;
        next = {
          submissionId,
          thought,
          sourceDraftRevision: r.sourceDraftRevision as number,
          attemptCount: r.attemptCount as number,
          lastAttemptAt,
          lastError: r.lastError as string | null,
          retryRequested: r.retryRequested as boolean,
          thoughtPersisted: r.thoughtPersisted as boolean,
          updatedAt,
          status: r.status as ThoughtSaveOutboxStatus,
        };
      } catch {
        continue;
      }
      const existing = (await outbox.readAll()).find(
        (entry) => entry.submissionId === next.submissionId,
      );
      if (existing === undefined) await outbox.insert(next);
      else if (!sameOutboxRecord(existing, next))
        throw new Error(
          `conflicting legacy outbox record: ${next.submissionId}`,
        );
    }
  }
