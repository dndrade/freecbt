...
import { FlowAction, FlowProgress, StandardScreen, backHeaderAction } from '@/components';
import { Reminders, useReminders } from '@/features/reminders/use-reminders';
                    ...
                    <Carousel
                        ...
                        renderItem={({ item, index }) => {
                            ...
                            return (
                                ...
                                >
                                    <OnboardingItem step={item} reminders={reminders} />
                                </View>
                            );
                        }}
                        ...
                        onSnapToItem={(index) => {
                            ...
                            if (announcedIndex.current !== index) {
                                ...
                                AccessibilityInfo.announceForAccessibility(t(slides[index].titleKey));
                            }
                            Keyboard.dismiss();
                        }}
...
function OnboardingItem({
    ...
    switch (step.presentation) {
        case 'guide':
            return <OnboardingPage step={step} translate={t} variation={<GuideVariation />} />;
        case 'informational':
        ...
        case 'reminders':
            return (
                <OnboardingPage
                    step={step}
                    translate={t}
                    variation={<ReminderVariation reminders={reminders} />}
                />
            );
        default:
            throw new Error(`unknown presentation: ${(step as any).presentation}`);
    }
}
...
function ReminderVariation({ reminders }: { reminders: Reminders }) {
    const i18n = useI18n();
    const t = (key: string, opts?: any) => i18n.t(key, opts);
